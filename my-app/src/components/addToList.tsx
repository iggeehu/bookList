...
import {createNewList, addToList, listObject, getLists} from '../redux/myListsSlice'
...
export const AddToList: FC<propType> = (props:propType) => {
    ...
    const createListPopUp = () => {
        if(createOrNot){
            return(
                ...
                <input className="" onChange={changeHandler} type='text' placeholder='Type list name' ref={inputRef} autoFocus/>
                <button onClick={createList} className="hover:bg-slate-500 rounded"><FontAwesomeIcon icon={solid('check')} />
                </button>
                </div>
            )}}
    ...
    const listDisplay = myLists.map(elem=>{
        return (
            <li className="ease-in-out p-1 duration-300 hover:bg-slate-200 rounded-md cursor-pointer hover:font-bold" onClick={addToListInComponent} key={elem.listID} id={elem.listID}>
            {elem.listTitle}
            </li>
        )
        }) 
    ...
    return (
        <div className="bg-slate-400 p-3 rounded-lg transition ease-in-out">
            <div className = "p-1 ease-in-out duration-300 hover:font-bold cursor-default" onClick={()=>{toggleCreateOrNot(!createOrNot)}}>Create New List</div>
                <div className="">{createListPopUp()}</div>
                <ul className="block">{myLists.length!==0? listDisplay: <li>You don't have any list yet</li>}</ul>
                <div className="">{addSuccessful==='successful'? <p>The book is added to the list</p>:null}</div>
