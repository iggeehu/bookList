...
export const Home: FC = () => {
  ...
  const booklist = () =>
    result.map((elem) => {
      return (
        <li
          className="grid grid-cols-4 min-h-96 rounded-md transition ease-in-out hover:bg-slate-400 border-slate-50 p-2"
          key={elem.id}
        >
          <div
            className=""
            onClick={() => seeDetails(elem.id, navigate, ":fromSearch")}
          >
            <img
              className=""
              src={
          ...
          >
            ...
            {elem.volumeInfo.subtitle ? ":" + elem.volumeInfo.subtitle : ""}
          </p>
          <p className="">
            Authors: {elem.volumeInfo.authors}
            {elem.volumeInfo.authors ? ":" + elem.volumeInfo.authors : ""}
          </p>
          <div className="">
            <ButtonRendering className="" idElem={elem.id} myLists={myLists} />
          </div>
        </li>
      );
    });
  ...
  const displayList = () => {
    if (result === undefined) {
      return (
        <div className="">Can't find any title matching the keywords.</div>
      );
  ...
  return (
    <div
      className="flex bg-inherit h-full justify-center p-10 h-auto"
      onKeyDown={keySearch}
    >
      <div className="displayList">
        <div className="flex flex-col justify-center w-full">
          ...
          <div className="flex p-3">
            ...
            />
            <button
              className="p-2 bg-inherit rounded hover:bg-gray-200"
              onClick={search}
            >
              <FontAwesomeIcon
                size="lg"
