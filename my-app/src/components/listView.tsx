...
export const ListView: FC = () => {
  ...
  if (myLists.length === 0) {
    ...
    const listTitle = myLists.filter((elem) => elem.listID === listID)[0]
      .listTitle;
    ...
    const change = (e: React.SyntheticEvent) => {
      if ((e.target as any).name === "listCommentField") {
        dispatch(
          fieldChange({ name: "listCommentField", value: e.target.value })
        );
      }
    ...
    const bookDisplayWithNoBookOption = () => {
      if (
        myLists.filter((elem) => elem.listID === listID)[0].list.length === 0
      ) {
    ...
    return (
      <div id={listID} className="p-10 flex flex-col items-center">
        ...
        <div className="flex flex-row">
          ...
          <button
            onClick={(e) =>
              showActionWindow(e, "list", listTitle, editDisplay, toggleEdit)
            }
            className="border font-mono text-md rounded bg-slate-400 hover:bg-slate-300 p-1"
          >
            Edit
